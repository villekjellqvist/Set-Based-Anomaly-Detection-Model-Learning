...
\end{align}
\VK{Possibly, the whole problem could be represented in terms of the dual.
The one would need instead to save $F[k]^TE[k]^{-1}F[k]$ and $E[k]^{-1}F[k]$,
whose elements (I think) does not grow to infinity as $k \rightarrow \infty$.
At the cost of having to do matrix inversions, although maybe the matrix inversion
lemma could help.}
